...
export default function ManualEntry() {
  ...
  const inputStyle = {width:'100%',padding:'10px',borderRadius:'8px',border:'1px solid #ddd',fontSize:'13px',fontFamily:'sans-serif',boxSizing:'border-box' as const,marginBottom:'10px'}
  const labelStyle = {fontSize:'12px',color:'#999',fontWeight:'500' as const,marginBottom:'4px',display:'block' as const}
  const sectionStyle = {backgroundColor:'white',border:'1px solid #eee',borderRadius:'12px',padding:'16px',marginBottom:'12px'}
  ...
  return (
    <div style={{padding:'24px',maxWidth:'480px',margin:'0 auto',fontFamily:'sans-serif',paddingBottom:'40px'}}>
      <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'20px'}}>
        ...
        <h1 style={{fontSize:'20px',fontWeight:'500',margin:0}}>Manual Entry</h1>
      </div>
      ...
      <div style={sectionStyle}>
        ...
        <input value={form.industry} onChange={e => update('industry', e.target.value)} placeholder="e.g. Plastics, Textiles, Flowers" style={inputStyle} />
      ...
      <div style={sectionStyle}>
        ...
        <textarea value={form.note} onChange={e => update('note', e.target.value)} placeholder="Add a note about this contact..." style={{...inputStyle,minHeight:'80px',resize:'vertical'}} />
      </div>

      <button onClick={handleSave} disabled={saving} style={{width:'100%',padding:'14px',backgroundColor:saving?'#999':'#1D9E75',color:'white',border:'none',borderRadius:'8px',fontSize:'15px',fontWeight:'500',cursor:saving?'not-allowed':'pointer'}}>
